...
import { geminiGenerate } from "../providers/vertex-client"
...
/**
 * Planning-phase "destructive" tools.
 *
 * This is intentionally broader than runtime `riskClass === "destructive"`.
 * For planning/confirmation UX, any tool that is either truly destructive OR
 * requires sequential, state-changing execution should request confirmation.
 */
export const DESTRUCTIVE_TOOLS = new Set(
  getAllToolCapabilities()
    .filter((cap) => cap.riskClass === "destructive" || cap.executionMode === "sequential")
    .map((cap) => cap.name),
)
...
export async function buildAgentPlan(
  ...
  const destructiveAvailableTools = availableTools.filter((toolName) => DESTRUCTIVE_TOOLS.has(toolName))
  const nonDestructiveAvailableTools = availableTools.filter((toolName) => !DESTRUCTIVE_TOOLS.has(toolName))
...
- Mark isDestructive true for: ${destructiveAvailableTools.join(", ") || "none"}
- Mark isDestructive false for: ${nonDestructiveAvailableTools.join(", ") || "none"}
...
function parsePlanResponse(rawText: string, availableTools: string[]): AgentPlan {
  try {
    ...
    const requiresConfirmation = steps.some((step) => step.isDestructive)
