...
import {
  ...
  getVertexLocation,
  isVertexAI,
} from "../providers/vertex-auth"
import { getVertexLiveDirectWsUrl } from "../live/vertex"
 ...
 */
export async function vertexGeminiGenerate(
  model: string,
...
export async function geminiGenerate(
  ...
  return vertexGeminiGenerate(model, body, options)
}
 ...
 * Works with both Google AI Studio and Vertex AI.
 */
export async function vertexGeminiGenerateStream(
  model: string,
  body: Record<string, unknown>,
  options?: { signal?: AbortSignal }
): Promise<Response> {
...
export async function geminiGenerateStream(
  ...
  return vertexGeminiGenerateStream(model, body, options)
}
 ...
 * Works with both Google AI Studio and Vertex AI.
 */
export async function vertexGeminiEmbed(
  text: string,
  options?: { signal?: AbortSignal }
): Promise<Response> {
...
export async function geminiEmbed(
  ...
  return vertexGeminiEmbed(text, options)
}
 ...
 *     server-side relay in `workers/live/handler.ts` (deployed via the
...
export async function getGeminiLiveWsUrl(): Promise<string> {
  return getVertexLiveDirectWsUrl()
}
