...
export function buildSleepTtsPrompt(
  source: 'library' | 'last-generated' | 'breathing',
  text: string,
  story: Pick<SleepStory, 'mode' | 'title' | 'category'> | null,
): string {
...
export function splitSleepTtsText(text: string, maxChunkChars: number = MAX_SLEEP_TTS_CHUNK_CHARS): string[] {
...
export async function runSleepSessionsTtsRoute(req: NextRequest): Promise<Response> {
  const startTime = Date.now()

  const auth = await getAuthenticatedSleepSessionsUserId({
    onUnexpectedError: (error) => {
      logError('sleep-tts.auth_error', error)
    },
  })
  ...
  const requestBody = await parseSleepSessionsRequestBody(req, ttsReqSchema, 'Invalid JSON body')
  if (!requestBody.ok) {
    return requestBody.response
  }

  const { storyId, source, text: customText, story: inlineStory } = requestBody.data
