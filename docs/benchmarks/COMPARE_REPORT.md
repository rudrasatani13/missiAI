...
- The previous pass added too many `next/dynamic` boundaries at once: full-page wrappers in `/chat`, plus four separate chat overlay splits.
...
- `app/chat/page.tsx`: deferred runtime `import()` inside `resolveLiveSetup()` for `buildVoiceSystemPrompt`.
...
- Removed four independent chat overlay lazy boundaries; replaced them with one `ChatOptionalOverlays` split.
...
- The route bundles still retain real savings: Chat `-4.56%`, Voice `-4.55%` versus the original baseline.
...
- `pnpm bench -- --feature chat-core,voice-live --server-command "pnpm start" --port 3000 --timeout-ms 60000 --with-build`: passed
...
- Audit the heaviest chat/dashboard dependencies inside `ChatOptionalOverlays` for one more high-value extraction, but keep the boundary count low.
