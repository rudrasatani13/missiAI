...
//   - The browser opens wss://<origin>/api/v1/voice-relay.
...
import { getLiveTicketFromCookieHeader, verifyLiveTicket } from "../../lib/ai/live/ticket"
import { getEnv } from "../../lib/server/platform/env"
import { type CfWebSocketLike, openLiveWsUpstream } from "./upstream"
...
export async function handleLiveWs(
  ...
  const ticket = getLiveTicketFromCookieHeader(req.headers.get("cookie")) ?? ""
  if (!ticket) {
  ...
  const upstream = await openLiveWsUpstream()
  if (!upstream.ok) {
    return jsonError(upstream.status, upstream.code, upstream.message)
  }
  const { upstreamWs } = upstream
