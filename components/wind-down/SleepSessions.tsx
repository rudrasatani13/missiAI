...
export default function SleepSessions() {
  ...
  const [activeTab, setActiveTab] = useState<
    "tonight" | "custom" | "breathing" | "library"
  >("tonight");
  ...
  const handlePlayBreathing = async (
    ...
    >
      ...
      <div className="flex justify-start gap-2 mb-4 flex-wrap">
        <button
          ...
          className={`px-4 py-2 rounded-full text-xs transition-colors ${activeTab === "tonight" ? "bg-white/10 text-white border border-white/20" : "text-white/40 hover:text-white/60 hover:bg-white/5 border border-transparent"}`}
        >
          Personal
        </button>
        <button
          onClick={() => setActiveTab("custom")}
          className={`px-4 py-2 rounded-full text-xs transition-colors ${activeTab === "custom" ? "bg-white/10 text-white border border-white/20" : "text-white/40 hover:text-white/60 hover:bg-white/5 border border-transparent"}`}
        >
          Custom
        </button>
        <button
          onClick={() => setActiveTab("breathing")}
          className={`px-4 py-2 rounded-full text-xs transition-colors ${activeTab === "breathing" ? "bg-white/10 text-white border border-white/20" : "text-white/40 hover:text-white/60 hover:bg-white/5 border border-transparent"}`}
        >
          Breathing
        </button>
        <button
          onClick={() => setActiveTab("library")}
          className={`px-4 py-2 rounded-full text-xs transition-colors ${activeTab === "library" ? "bg-white/10 text-white border border-white/20" : "text-white/40 hover:text-white/60 hover:bg-white/5 border border-transparent"}`}
        >
          Library
        </button>
      </div>
      ...
      {activeTab === "tonight" && (
        <div className="w-full bg-black/20 border border-white/10 inset-shadow-sm rounded-3xl p-8 flex flex-col items-center justify-center text-center">
          ...
          <h3 className="text-lg text-white font-light tracking-wide mb-2">
            Tonight&apos;s Story
          </h3>
      ...
      {activeTab === "custom" && (
        <div className="w-full bg-black/20 border border-white/10 inset-shadow-sm rounded-3xl p-8 flex flex-col items-center justify-center text-center">
          ...
          <textarea
            ...
            rows={3}
          />
          <p className="w-full text-right text-xs text-white/30 mb-6">
        ...
        <div className="w-full flex flex-col gap-3">
          ...
          >
            <div>
              ...
              <p className="text-xs text-white/40">
                Inhale 4s, hold 7s, exhale 8s. Instantly calming.
              </p>
            </div>
          ...
          >
            <div>
              ...
              <p className="text-xs text-white/40">
                Hold and exhale equally. Reset your mind.
              </p>
            </div>
            <Play className="w-5 h-5 text-white/20" />
          </button>
          ...
          >
            <div>
              ...
              <p className="text-xs text-white/40">
                Deep inhale, slow exhale. For profound rest.
              </p>
            </div>
            <Play className="w-5 h-5 text-white/20" />
          </button>
          ...
          {library.map((story) => (
            ...
            >
              <div>
                ...
                <h4 className="text-sm font-light text-white leading-relaxed mb-2">
                  {story.title}
                </h4>
                <p className="text-xs text-white/30 line-clamp-3 leading-loose">
                  {story.text}
                </p>
              </div>
              <div className="flex items-center gap-2 mt-4 text-xs text-amber-300/50">
                <Play className="w-3 h-3" />{" "}
                {formatTime(story.estimatedDurationSec)}
              </div>
            </button>
          ))}
        </div>
