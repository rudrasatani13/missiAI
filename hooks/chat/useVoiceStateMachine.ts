...
export function shouldAutoSaveVoiceMemory(options: {
  userId?: string
  conversationLength: number
  incognito: boolean
}) {
  return Boolean(options.userId) && options.conversationLength >= 4 && !options.incognito
}

export function buildVoiceMemoryAutoSavePayload(
  ...
  return JSON.stringify({
    ...
    interactionCount: conversation.filter((message) => message.role === "user").length,
    incognito,
    analyticsOptOut,
  })
}

export function getVoiceMemoryAutoSavePrivacyState(options: {
  incognitoRef?: React.MutableRefObject<boolean>
  analyticsOptOutRef?: React.MutableRefObject<boolean>
}) {
  return {
    incognito: !!options.incognitoRef?.current,
    analyticsOptOut: !!options.analyticsOptOutRef?.current,
  }
}

export function triggerVoiceMemoryAutoSave(options: {
  userId?: string
  conversation: ConversationEntry[]
  incognitoRef?: React.MutableRefObject<boolean>
  analyticsOptOutRef?: React.MutableRefObject<boolean>
  ...
  const privacyState = getVoiceMemoryAutoSavePrivacyState({
    incognitoRef: options.incognitoRef,
    analyticsOptOutRef: options.analyticsOptOutRef,
  })

  if (!shouldAutoSaveVoiceMemory({
    userId: options.userId,
    conversationLength: options.conversation.length,
    incognito: privacyState.incognito,
  })) {
    return false
  }

  const payload = buildVoiceMemoryAutoSavePayload(
    options.conversation,
    privacyState.incognito,
    privacyState.analyticsOptOut,
  )
  const request = (options.fetchImpl ?? fetch)("/api/v1/memory", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: payload,
  })

  void request.catch((autosaveError) => {
    console.error("[voice-memory-autosave] Failed to save conversation memory", autosaveError)
  })

  return true
}

export function saveVoiceMemoryBeacon(options: {
  userId?: string
  conversation: ConversationEntry[]
  incognitoRef?: React.MutableRefObject<boolean>
  analyticsOptOutRef?: React.MutableRefObject<boolean>
  sendBeacon?: (url: string, data: BodyInit | null | undefined) => boolean
}) {
  if (!options.userId || options.conversation.length < 2) {
    return false
  }

  if (options.incognitoRef?.current) {
    return false
  }

  const interactionCount = options.conversation.filter((message) => message.role === "user").length
  const analyticsOptOut = !!options.analyticsOptOutRef?.current

  let messages = options.conversation
  ...
  if (payload.length >= 60_000) {
    messages = options.conversation.slice(-6)
...
export function getVoiceMemoryAutoSaveErrorMessage(error: unknown) {
...
export function buildVoiceMemoryAutoSaveErrorPayload(options: {
  error: unknown
  conversationLength: number
  ...
  return JSON.stringify({
    ...
    message: getVoiceMemoryAutoSaveErrorMessage(options.error),
    metadata: {
...
export async function reportVoiceMemoryAutoSaveFailure(options: {
  error: unknown
  conversationLength: number
  interactionCount: number
}) {
  console.error("[voice-memory-autosave] Failed to save conversation memory", options.error)

  toast.error("Couldn't save this conversation to memory.", {
  ...
  try {
    const response = await fetch("/api/v1/client-errors", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: buildVoiceMemoryAutoSaveErrorPayload(options),
      keepalive: true,
    })
    ...
    console.error("[voice-memory-autosave] Failed to report autosave error", reportError)
  }
}
...
export function useVoiceStateMachine(options: UseVoiceStateMachineOptions) {
  ...
  const speakText = useCallback(
    ...
    [startTTSMonitor, stopTTSMonitor, continueOrReset, resetToIdle],
  )
