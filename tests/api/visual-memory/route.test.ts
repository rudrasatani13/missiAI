...
describe('GET /api/v1/visual-memory', () => {
  ...
  it('falls back to the default limit when the query param is invalid', async () => {
    const req = new NextRequest('http://localhost/api/v1/visual-memory?limit=abc')
    const res = await GET(req)

    expect(res.status).toBe(200)
    expect(mockGetVisualRecords).toHaveBeenCalledWith(expect.anything(), 'user-1', 20)
  })
...
describe('visual-memory catch-all dispatch', () => {
  it('returns 404 for unknown route segments', async () => {
    const req = new NextRequest('http://localhost/api/v1/visual-memory/unknown')
