...
beforeEach(() => {
  ...
  mockCheckVoiceLimit.mockResolvedValue({ allowed: true, usedSeconds: 0, limitSeconds: 100, remainingSeconds: 0 })
...
describe("POST /api/v1/live-token", () => {
  ...
  it("returns 429 if rate limit is exceeded", async () => {
    mockCheckRateLimit.mockResolvedValue({ allowed: false, limit: 10, remaining: 0, resetAt: 0, retryAfter: 0 })
