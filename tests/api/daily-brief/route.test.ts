...
const GET_UNKNOWN = (req: NextRequest) => _GET(req, { params: Promise.resolve({ path: ['unknown'] }) } as any)
...
describe('GET /api/v1/daily-brief', () => {
  ...
  it('returns 500 when KV binding is unavailable', async () => {
    mockGetRequestContext.mockReturnValue({ env: {}, ctx: {} as any, cf: {} as any } as any)
    ...
    expect(body).toEqual({ success: false, error: 'Service unavailable', code: 'INTERNAL_ERROR' })
  })
})
...
describe('POST /api/v1/daily-brief', () => {
  ...
  it('passes tz and hour query values into the generation context', async () => {
    mockGetTodaysBrief.mockResolvedValueOnce(null)
    mockBuildContext.mockResolvedValueOnce({
      userName: 'Test',
      topGoals: [],
    ...
    const req = makeRequest('POST', '/api/v1/daily-brief?tz=Asia/Kolkata&hour=7')
    const res = await POST(req)

    expect(res.status).toBe(201)
    expect(mockGenerateBrief).toHaveBeenCalledWith(expect.objectContaining({ timezone: 'Asia/Kolkata', localHour: 7 }))
  })
})
...
describe('PATCH /api/v1/daily-brief/tasks/[taskId]', () => {
  ...
  it('returns 400 for an invalid task id', async () => {
    const req = new NextRequest('http://localhost/api/v1/daily-brief/tasks/way-too-long', {
      method: 'PATCH',
    })
    const res = await PATCH(req, { params: Promise.resolve({ taskId: 'x'.repeat(21) }) })
    const body = await res.json()
...
describe('daily-brief catch-all dispatch', () => {
  it('returns 404 for unknown route segments', async () => {
    const req = makeRequest('GET', '/api/v1/daily-brief/unknown')
    const res = await GET_UNKNOWN(req)
    const body = await res.json()
