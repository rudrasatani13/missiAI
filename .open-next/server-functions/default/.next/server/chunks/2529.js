"use strict";exports.id=2529,exports.ids=[2529],exports.modules={11910:(a,b,c)=>{function d(a){if(!a)return a;let b=a.replace(/<(script|style)[^>]*>[\s\S]*?<\/\1>/gi,"");return(b=b.replace(/<[^>]+>/g,"")).trim()}function e(a){var b,c;if(!a)return"";let e=d(a);return(c=e=(b=e)?b.replace(/[\0\x08\x09\x1a\n\r"'\\]/g,a=>{switch(a){case"\0":return"\\0";case"\b":return"\\b";case"	":return"\\t";case"\x1a":return"\\z";case"\n":return"\\n";case"\r":return"\\r";case'"':return'\\"';case"'":return"\\'";case"\\":return"\\\\";default:return a}}):b)?c.normalize("NFKC").replace(/[\u200B-\u200D\uFEFF\u2060\u00AD]/g,"").replace(f,"").trim():c}c.d(b,{QZ:()=>d,o2:()=>e});let f=/ignore\s*(all\s*)?previous\s*(instructions|prompts)?|you are now|system\s*:|<\|.*?\|>|\[INST\]|\[\/INST\]|<\|system\|>|<\|user\|>|<\|assistant\|>|forget\s*(all\s*)?(previous|prior|above)\s*(instructions|context)?|disregard\s*(all\s*)?(previous|prior|above)/gi},20279:(a,b,c)=>{c.d(b,{B9:()=>f,D0:()=>g,l3:()=>e});var d=c(66737);async function e(a){let b=await (0,d.$)(),c=await b.users.getUser(a),e=c.publicMetadata?.plan;return"plus"===e||"pro"===e?e:"free"}async function f(a,b,c){let e=await (0,d.$)(),f=(await e.users.getUser(a)).publicMetadata??{};await e.users.updateUser(a,{publicMetadata:{...f,plan:b,...c?.dodoCustomerId!==void 0&&{dodoCustomerId:c.dodoCustomerId},...c?.dodoSubscriptionId!==void 0&&{dodoSubscriptionId:c.dodoSubscriptionId},...c?.currentPeriodEnd!==void 0&&{currentPeriodEnd:c.currentPeriodEnd},...c?.cancelAtPeriodEnd!==void 0&&{cancelAtPeriodEnd:c.cancelAtPeriodEnd}}})}async function g(a){let b=await (0,d.$)(),c=(await b.users.getUser(a)).publicMetadata;return{userId:a,planId:c?.plan??"free",dodoCustomerId:c?.dodoCustomerId,dodoSubscriptionId:c?.dodoSubscriptionId,currentPeriodEnd:c?.currentPeriodEnd,cancelAtPeriodEnd:c?.cancelAtPeriodEnd,updatedAt:Date.now()}}},35235:(a,b,c)=>{c.d(b,{E8:()=>j,Ex:()=>h});var d=c(12818);let e={assistant:`You are Missi — a smart, honest AI assistant that genuinely helps. You have real-time internet search.
...
- ALWAYS complete your full answer.`,bestfriend:`You are Missi — an AI voice assistant and the user's smart, caring best friend. You have real-time internet search through Google Search.
...
- ALWAYS finish your complete thought — never stop mid-sentence.`,professional:`You are Missi — a sharp, no-nonsense AI executive assistant. You have real-time internet search.
...
- No bullet points, lists, markdown, formatting, emojis, URLs.
- ALWAYS complete your full answer.`,playful:`You are Missi — a witty, fun AI voice assistant with real personality. You have real-time internet search.
...
- Spoken aloud by TTS.
- No bullet points, lists, markdown, formatting, emojis, URLs.
- ALWAYS complete your answer.`,mentor:`You are Missi — a wise, thoughtful AI mentor who actually pushes people to grow. You have real-time internet search.
...
- Spoken aloud by TTS.
- No bullet points, lists, markdown, formatting, emojis, URLs.
- ALWAYS complete your full thought.`},f=`
LANGUAGE RULES — CRITICAL (NEVER VIOLATE):
- User may speak in ANY language: Hindi, Hinglish, English, or mixed. Input may be garbled by speech-to-text.
- You understand ALL input regardless of language.
...
- Spoken aloud by TTS — write as you would speak.
- No bullet points, lists, markdown, formatting, emojis, URLs
- ALWAYS complete your full answer`,g={gemini:"gemini-2.5-pro",openai:"gpt-4o",claude:"claude-sonnet-4-6"};function h(a,b,c){let d="";return(d="custom"===a&&c?.trim()?`${c.trim()}

${f}`:e[a]??e.assistant,b?.trim())?`${d}

${b.trim()}`:d}let i={gemini:async function a(a){let b=a.messages.map(a=>({role:"assistant"===a.role?"model":"user",parts:[{text:a.content}]})),c={system_instruction:{parts:[{text:a.systemPrompt}]},contents:b,generationConfig:{temperature:a.temperature,topP:.95,topK:40,maxOutputTokens:a.maxOutputTokens}};a.useGoogleSearch&&(c.tools=[{google_search:{}}]);let e=new AbortController,f=setTimeout(()=>e.abort(),a.timeoutMs);try{let b=await (0,d.geminiGenerate)(a.model,c,{signal:e.signal});if(!b.ok){let a=await b.text();throw Error(`Gemini API error ${b.status}: ${a}`)}var g=await b.json();let f=g?.candidates?.[0]?.content?.parts;return f?f.filter(a=>"string"==typeof a.text).map(a=>a.text).join("").trim():""}finally{clearTimeout(f)}},openai:async function(a){let b=process.env.OPENAI_API_KEY;if(!b)throw Error("OPENAI_API_KEY is not configured");let c=new AbortController,d=setTimeout(()=>c.abort(),a.timeoutMs);try{let d=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${b}`},body:JSON.stringify({model:a.model,messages:[{role:"system",content:a.systemPrompt},...a.messages.map(a=>({role:a.role,content:a.content}))],temperature:a.temperature,max_tokens:a.maxOutputTokens}),signal:c.signal});if(!d.ok){let a=await d.text();throw Error(`OpenAI API error ${d.status}: ${a}`)}let e=await d.json();return e?.choices?.[0]?.message?.content?.trim()??""}finally{clearTimeout(d)}},claude:async function(a){let b=process.env.ANTHROPIC_API_KEY;if(!b)throw Error("ANTHROPIC_API_KEY is not configured");let c=new AbortController,d=setTimeout(()=>c.abort(),a.timeoutMs);try{let d=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":b,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:a.model,system:a.systemPrompt,messages:a.messages.map(a=>({role:a.role,content:a.content})),temperature:a.temperature,max_tokens:a.maxOutputTokens}),signal:c.signal});if(!d.ok){let a=await d.text();throw Error(`Claude API error ${d.status}: ${a}`)}let e=await d.json();return e?.content?.[0]?.text?.trim()??""}finally{clearTimeout(d)}}};async function j(a,b,c){let d=c?.provider??"gemini",e=i[d];if(!e)throw Error(`Unknown AI provider: ${d}`);return e({messages:[{role:"user",content:b}],systemPrompt:a,model:c?.model??g[d],temperature:c?.temperature??.3,maxOutputTokens:c?.maxOutputTokens??1024,timeoutMs:c?.timeoutMs??3e4,useGoogleSearch:!1})}},62471:(a,b,c)=>{c.d(b,{Fi:()=>e,Qz:()=>g,v3:()=>f});var d=c(44676);async function e(){let{userId:a}=await (0,d.j)();if(!a)throw new f;return a}class f extends Error{constructor(){super("Unauthorized"),this.status=401,this.name="AuthenticationError"}}function g(){return new Response(JSON.stringify({success:!1,error:"Unauthorized",code:"UNAUTHORIZED"}),{status:401,headers:{"Content-Type":"application/json"}})}},77454:(a,b,c)=>{c.d(b,{_:()=>i,w:()=>j});var d=c(35234),e=c(84331);let f=d.vk(a=>{if("string"!=typeof a||""!==a.trim())return a},d.Yj().optional()),g=d.vk(a=>{if("string"!=typeof a||""!==a.trim())return a},d.Yj({required_error:"Missing required environment variable"})),h=d.Ik({ELEVENLABS_API_KEY:g,ELEVENLABS_VOICE_ID:f,ELEVENLABS_VOICE_CALM:f,ELEVENLABS_VOICE_COACH:f,ELEVENLABS_VOICE_FRIEND:f,ELEVENLABS_VOICE_BOLLYWOOD:f,ELEVENLABS_VOICE_DESI_MOM:f,ELEVENLABS_SLEEP_VOICE_ID:f,CLERK_SECRET_KEY:g,DAILY_BUDGET_USD:d.vk(a=>{if("string"==typeof a){let b=parseFloat(a);return isNaN(b)?5:b}return 5},d.ai()),NODE_ENV:d.Yj().default("production"),DODO_PAYMENTS_API_KEY:g,DODO_WEBHOOK_SECRET:g,DODO_PRO_PRODUCT_ID:g,DODO_PAYMENTS_MODE:d.Yj().default("live_mode"),VAPID_PRIVATE_KEY:f,GOOGLE_CLIENT_ID:f,GOOGLE_CLIENT_SECRET:f,NOTION_CLIENT_ID:f,NOTION_CLIENT_SECRET:f,NOTION_API_KEY:f,APP_URL:d.vk((a,b)=>"http://localhost:3000",d.Yj()),AI_BACKEND:d.k5(["vertex","google-ai"]).default("vertex"),VERTEX_AI_PROJECT_ID:f,VERTEX_AI_LOCATION:f,GOOGLE_SERVICE_ACCOUNT_JSON:f,RESEND_API_KEY:f});function i(){try{return h.parse(process.env)}catch(a){if(a instanceof e.G){let b=a.issues.map(a=>a.path.join(".")).join(", ");throw Error(`Missing or invalid required environment variable(s): ${b}`)}throw a}}function j(a){let b=process.env[a];return"string"==typeof b&&""!==b.trim()}}};